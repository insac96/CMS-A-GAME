...
<template>
  <UContainer class="lg:pb-28 pb-16">
    <UiFlex class="LimitedEventPage__Title" type="col" justify="center">
      ...
      <UiText class="LimitedEventPage__Title__Text sm:text-3xl text-xl sm:mb-2" weight="bold">Liên Nạp</UiText>
    </UiFlex>

    <UiFlex justify="center" class="gap-6 mt-20" wrap>
      <UiFlex class="LimitedEventPayment lg:w-[14.7875354vw] lg:h-[15vw] w-[29.5750708vw] h-[30vw]" justify="center" @click="openReceive(event)" v-for="event in list" :key="event._id">
        <UiFlex justify="center" class="LimitedEventPayment__Text lg:h-[2.5vw] h-[5vw] lg:text-xl">Ngày {{ event.need }}</UiFlex>
      </UiFlex>
    </UiFlex>
...
const submit = async () => {
  try {
    ...
    await useAPI('limitedevent/payment/receive', JSON.parse(JSON.stringify(state.value)))
...
const getList = async () => {
  try {
    const data = await useAPI('limitedevent/payment/list')
    list.value = data
  }
  catch(e){
    list.value = []
