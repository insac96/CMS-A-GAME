<template>
  <UiFlex :items="!configStore.config.logo_image ? 'end' : 'center'">
    <svg class="w-auto md:h-9 h-6" width="264" height="264" viewBox="0 0 264 264" fill="none" xmlns="http://www.w3.org/2000/svg" v-if="!configStore.config.logo_image">
    ...
    <img v-else :src="useMakeLink().imgLink(configStore.config.logo_image)" class="w-auto h-[calc(var(--header-size)*0.6)] mr-2">

    <UiText class="md:text-lg text-sm md:inline hidden" weight="bold" v-if="!configStore.config.logo_image">{{ text[0] }}</UiText>
    <UiText class="md:text-lg text-sm md:inline hidden" weight="bold" color="primary" v-if="!configStore.config.logo_image">{{ text[1] }}</UiText>
  </UiFlex>
</template>
