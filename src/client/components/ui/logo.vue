<template>
  <UiFlex :items="!configStore.config.logo_image ? 'end' : 'center'">
    ...
    <img v-else :src="useMakeLink().imgLink(configStore.config.logo_image)" class="w-auto h-[calc(var(--header-size)*0.9)] mr-2">
    <UiText class="md:text-lg text-sm md:inline hidden" weight="bold">{{ text[0] }}</UiText>
    <UiText class="md:text-lg text-sm md:inline hidden" weight="bold" color="primary">{{ text[1] }}</UiText>
  </UiFlex>
</template>
