import type { IAuth, IDBItem, IDBLunaPayMission, IDBUser } from '~~/types'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Event
    const eventData = await DB.LunaPayMission
    .findOne({ _id: payment })
    .populate({
      path: 'gift.item',
      select: 'item_id type'
    })
    .select('-createdAt -updateAt') as IDBLunaPayMission
    ...
    if(giftItem.length > 0){
      await gameSendMail(event, {
        ...
        role_id: role,
        title: 'Liên Nạp Tết',
        content: 'Vật phẩm nhận từ sự kiện liên nạp tết',
        items: giftItem
      })
    }
    ...
    logUser(event, auth._id, `Nhận thưởng mốc <b>${eventData.need.toLocaleString('vi-VN')}</b> của sự kiện <b>liên nạp tết</b> tại máy chủ <b>${server}</b> nhân vật <b>${role}</b>`)
    if(change.length > 0) logUser(event, auth._id, `Nhận <b>${change.join(', ')}</b> từ mốc <b>${eventData.need.toLocaleString('vi-VN')}</b> của sự kiện <b>liên nạp tết</b>`)
