import type { IAuth, IDBItem, IDBLunaEgg, IDBLunaPayment, IDBUser } from '~~/types'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // @ts-expect-error
    const userEggRowDown = user.lunanewyear.egg[rowDown]
    // @ts-expect-error
    const userEggRowUp = user.lunanewyear.egg[rowUp]
    ...
    // @ts-expect-error
    user.lunanewyear.egg[rowSelect].push({
      item: item._id,
      amount: resultGift.amount,
      index: index
    })
