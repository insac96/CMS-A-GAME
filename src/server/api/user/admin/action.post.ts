...
export default defineEventHandler(async (event) => {
  try {
    ...
    if(type == 'pay.day'){
      ...
      await logAdmin(event, 'Reset <b>tích nạp ngày</b> tất cả tài khoản', auth._id)
    }
    if(type == 'pay.month'){
      await logAdmin(event, 'Reset <b>tích nạp tháng</b> tất cả tài khoản', auth._id)
      await DB.User.updateMany({}, { 'pay.month.money' : 0, 'pay.month.count': 0 })
    }
    if(type == 'pay.total'){
      await logAdmin(event, 'Reset <b>tích nạp tổng</b> tất cả tài khoản', auth._id)
      await DB.User.updateMany({}, { 'pay.total.money' : 0, 'pay.total.count': 0 })
    }

    if(type == 'spend.day'){
      await DB.User.updateMany({}, { 'spend.day.coin' : 0, 'spend.day.count': 0 })
      await logAdmin(event, 'Reset <b>tiêu phí ngày</b> tất cả tài khoản', auth._id)
    }
    if(type == 'spend.month'){
      await logAdmin(event, 'Reset <b>tiêu phí tháng</b> tất cả tài khoản', auth._id)
      await DB.User.updateMany({}, { 'spend.month.coin' : 0, 'spend.month.count': 0 })
    }
    if(type == 'spend.total'){
      await logAdmin(event, 'Reset <b>tiêu phí tổng</b> tất cả tài khoản', auth._id)
